...
export default function OverbookingSettings() {
  ...
  const selectedSlot = useSelector((state) => state.overbooking.timeSlot);
  ...
  return (
    <>
      ...
      <Card
        title="Tỷ lệ Overbooking theo Ngày trong tuần"
        style={{ marginTop: 32 }}
      >
        {/* <Row gutter={16}>
          <Col xs={24} md={16}>
            <div>
              <canvas id="overbookingChart" height="300"></canvas>
            </div>
          </Col>
          <Col xs={24} md={8}>
            <Card title="Chi tiết khung giờ" style={{ height: "100%" }}>
              {selectedSlot ? (
                <>
                  <p>
                    <strong>
                  ...
                  <p>Tổng đơn đặt bàn: {selectedSlot.totalBookings}</p>
                  <p>
                    Đơn hủy / No-show: {selectedSlot.canceledOrNoShowBookings}
                  </p>
                  <p>Tỷ lệ overbooking: {selectedSlot.overbookingRate}%</p>
                  <p
                    style={{
                      color:
                        selectedSlot.overbookingRate > 15 ? "red" : "green",
                    }}
                  >
                    {selectedSlot.overbookingRate > 15
                      ? "Tỷ lệ cao, nên tăng overbooking"
                      : "Tỷ lệ thấp, có thể giữ nguyên"}
                  </p>
                </>
              ) : (
                <p className="text-muted">
                  Chọn một khung giờ trên biểu đồ để xem chi tiết.
                </p>
              )}
            </Card>
          </Col>
        ...
        <Table
          style={{ marginTop: 24 }}
          dataSource={selectedSlot}
          rowKey={(record) => `${record.dayOfWeek}`}
          pagination={false}
          columns={[
            {
              ...
              render: (dayOfWeek) => {
                ...
                return days[dayOfWeek - 1] || "-"; // Chuyển dayOfWeek (1-7) thành tên ngày
              },
            },
            {
            ...
            {
              title: "Đơn hủy / No-show",
              dataIndex: "canceledOrNoShowBookings",
              key: "canceledOrNoShowBookings",
            },
            {
              title: "Tỷ lệ overbooking",
              dataIndex: "overbookingRate",
              key: "overbookingRate",
              render: (text) => (
                <span style={{ color: text > 15 ? "red" : "inherit" }}>
                  {text}%
                </span>
              ),
            },

...
export const calculateDistance = (myCoords, restaurantCoords) => {
    if (!myCoords?.latitude || !myCoords?.longitude || !restaurantCoords?.lat || !restaurantCoords?.lng) {
        return null;
    }
    ...
    return (
        ...
        >
            <div className={styles.imageContainer}>
                ...
                />
                {restaurant.discount && <div className={styles.discountBadge}>Giảm {restaurant.discount}</div>}
            </div>
            ...
            <div className={styles.restaurantDetails}>
                ...
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={styles.priceLevel}>{"$".repeat(restaurant.priceLevel)}</span>
...
const RestaurantGrid = () => {
    ...
    const restaurantSearch = useSelector((state) => state.search.restaurantsSearch);
    ...
    const sortedRestaurants = [...temp_restaurantSearch].sort((a, b) => {
        const distanceA =
            calculateDistance({ longitude: 106.6983125, latitude: 10.7802256 }, { lat: a.viDo, lng: a.kinhDo }) ||
            Infinity;
        const distanceB =
            calculateDistance({ longitude: 106.6983125, latitude: 10.7802256 }, { lat: b.viDo, lng: b.kinhDo }) ||
            Infinity;

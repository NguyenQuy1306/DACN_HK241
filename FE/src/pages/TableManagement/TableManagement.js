...
function TableManagement() {
    ...
    const twoWeeksLater = new Date(Date.now() + 12096e5).toISOString().split("T")[0];
    ...
    const deleteTableResposne = useSelector((state) => state.table.deleteTableResposne);
    ...
    useEffect(() => {
        if (restaurantOwner) {
            console.log("call getTableForRestaurantByOwner", restaurantOwner.maSoNhaHang);
            dispatch(
                getTableForRestaurantByOwner({
                ...
                <div className={styles.tableItems}>
                    {tableList && tableList.length > 0
                        ? Object.keys(groupTablesByDateAndTime(tableList)).map((date) => (
                              <div
                                  key={date}
                                  className={styles.tableDateGroup}
                              >
                                  <h5 className={styles.dateTitle}>{date}</h5>
                                  {Object.keys(groupTablesByDateAndTime(tableList)[date]).map((time) => (
                                      <div
                                          key={time}
                                          className={styles.timeGroup}
                                      >
                                          <h6 className={styles.timeTitle}>{time}</h6>
                                          {groupTablesByDateAndTime(tableList)[date][time].map((table) => (
                                              <div
                                                  key={table.maSo.thuTuBan}
                                                  className={styles.tableItem}
                                              >
                                                  <div className={styles.tableInfo}>
                                                      <div className={styles.tableDate}>{table.ngay}</div>
                                                      <div className={styles.tableDetails}>
                                                          {table.gio} - {table.soNguoi} người - Số lượng:{" "}
                                                          {table.soLuong}
                                                      </div>
                                                  </div>
                                                  <button

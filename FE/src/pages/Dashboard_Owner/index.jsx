...
function Dashboard_Owner() {
    ...
    const processRevenueData = (orders) => {
        ...
        orders.forEach((order) => {
            const day = getDayOfWeek(order.ngay);
            const revenue = order.tongTienThanhToan || 0; // Use the total amount if available
                    ...
                    >
                        ...
                        <Col span={6}>
                            <Statistic
                                ...
                                quantity={
                                    messages
                                        ? messages.filter((item) => item.trangThai === "CANCELLED_REFUNDED").length
                                        : 0
                                }
                                up={false}
                                rate={12}
                        ...
                        <Col span={6}>
                            <Statistic
                                ...
                                quantity={formatCurrency(messages.reduce((acc, cur) => acc + cur.tongTienThanhToan, 0))}
                                up={true}
                                rate={13}
                                compare="So với hôm qua"
                            />

...
export const getTableForRestaurant = async (params) => {
    try {
        console.log("params in al ", await API.get(`api/table/restaurant`, { params }));
...
export const getRateInRestaurant = async (params) => {
    try {
        const response = await API.get(`/api/rate/restaurant/${params.restaurantId}`);
        return response.data;
    } catch (error) {
        throw error.response?.data || error;
    }
...
export const createPaymentLink = async ({ deposit, request, RETURN_URL }) => {
    try {
        ...
        const queryParams2 = RETURN_URL ? `&returnUrl=${encodeURIComponent(RETURN_URL)}` : "";
...
export const updateFavoriteList = async ({ listId, listName }) => {
    try {
        const response = await API.post(`/api/favorite-list/update/${listId}`, listName, {
            headers: {
...
export const updateCountOfTable = async (params) => {
    try {
        const response = await API.put(`/api/table?restaurantId=${params.restaurantId}&thuTuBan=${params.thuTuBan}`);
        return response.data;
    } catch (error) {
        throw error.response?.data || error;
    }
...
export const createFood = async ({ restaurantId, categoryId, foodRequest, file }) => {
    try {
        const formData = new FormData();
        formData.append("foodRequest", new Blob([JSON.stringify(foodRequest)], { type: "application/json" })); // Convert JSON to Blob
        formData.append("file", file);

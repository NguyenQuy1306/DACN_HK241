...
export const getFoodById = createAsyncThunk("/food/id", async ({ restaurantId, foodId }, { rejectWithValue }) => {
    try {
...
export const getFoodByCategory = createAsyncThunk(
    ...
    async ({ restaurantId, categoryId }, { rejectWithValue }) => {
        try {
            const response = await api.getFoodByCategory({ restaurantId, categoryId });
            return response.payload;
        } catch (error) {
            return rejectWithValue(error.response.data);
        }
...
export const updateFood = createAsyncThunk("/updateFood", async (params, { rejectWithValue }) => {
    try {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getFood.fulfilled, (state, action) => {
                state.loading = false;
                let foodResult = [];
                foodResult = action.payload.reduce((acc, cur) => [...acc, ...cur.foodResponses], []);

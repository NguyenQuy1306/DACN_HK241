...
export const getFoodforOwmer = createAsyncThunk(
  "/getFoodforOwmer",
  async (params, { rejectWithValue }) => {
    try {
      const response = await api.getFood(params);
...
export const foodSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getFoodforOwmer.pending, (state) => {
        state.loading = true;
        state.food = [];
      })
      .addCase(getFoodforOwmer.fulfilled, (state, action) => {
        state.loading = false;
        let foodResult = [];
        ...
        state.foodforOwmer = foodResult;
      })
      .addCase(getFoodforOwmer.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })

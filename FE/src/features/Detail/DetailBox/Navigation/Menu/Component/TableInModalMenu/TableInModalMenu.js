...
export default function CustomizedTables({ combo }) {
    let totalPrice = 0;
    console.log("commbo in modal test:", combo);
    const comboType = useSelector((state) => state.combo.comboType);

    let listFood = [];
    console.log("COMBO DA CHON", combo);
    console.log("comboTypecomboType", comboType);
    ...
    console.log("comboType !== ", comboType !== "newCombo");
    return (
        <TableContainer
            component={Paper}
        ...
        >
            ...
            >
                <TableHead>
                    <TableRow>
                        <StyledTableCell className="StyledTableCell1">Tên món ăn</StyledTableCell>
                        <StyledTableCell
                            align="left"
                            className="StyledTableCell2"
                        >
                            Giá
                        </StyledTableCell>
                    ...
                    {comboType === "newCombo" &&
                        listFood.map((row) => (
                            <StyledTableRow key={row.ten}>
                                ...
                                <StyledTableCell align="left">{row.item.gia * row.quantity} đ</StyledTableCell>
                            </StyledTableRow>
                        ))}
                    ...
                    {comboType !== "availableCombo" &&
                        ...
                        combo.map((row) => (
                            <StyledTableRow key={row.item.ten}>
                                ...
                                <StyledTableCell align="left">{row.quantity}</StyledTableCell>
                                <StyledTableCell align="left">{row.quantity * row.item.gia} đ</StyledTableCell>
                            </StyledTableRow>
                        ))}

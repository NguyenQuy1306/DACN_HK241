...
import { getTableForRestaurant, setChoosedTable } from "../../../../../../redux/features/tableSlice";
...
const Bookingwidget = ({ selectedPlace }) => {
    ...
    useEffect(() => {
        if (selectedPlace && selectedPlace.maSoNhaHang) {
            ...
            dispatch(getTableForRestaurant({ restaurantId: selectedPlace.maSoNhaHang }));
        }
    ...
    let totalCost = 0;
    if (Array.isArray(menuChoosed) && menuChoosed.length > 0 && !menuChoosed[0].comboName) {
    ...
    const groupFoodsByCategory = (foods) => {
        return foods.reduce((acc, food) => {
            const categoryId = food.item?.danhMuc?.maSoDanhMuc || food.danhMuc?.maSoDanhMuc;
    ...
    // useEffect(() => {
    //     console.log(`CAC KHUNG GIO CO THE DAT TRONG NGAY ${date} la: ${timeTableAvailable} `);
    // }, [timeTableAvailable, date]);
    ...
    useEffect(() => {
        if (date && time && person) {
            ...
            const tables = tableAvailable
                .filter((item) => item.ngay === formattedDate && item.gio.slice(0, -3) === time)
                .map((item) => item.ban)
                .flat()
            ...
            <div className="BookingwidgetDiv">
                ...
                {openBookingWithMenu && (
                    <div class="BookingwidgetDiv_H1_new">
                        <div
                            data-testid="offer-tooltip"
                            class="BookingwidgetDiv_H1_new_h1"
                        >
                            <div class="BookingwidgetDiv_H1_new_h1_div1">
                                ...
                                <div class="BookingwidgetDiv_H1_new_h1_div_detail">
                                    <p class="BookingwidgetDiv_H1_new_h1_div_detail_p1">
                                        {menuChoosed[0].comboName ? menuChoosed[0].comboName : "Menu bạn đã tạo"}
                                    </p>
                                    <p
                                    ...
                                    >
                                        Chú ý: Bạn hãy tiến hành chọn các tiêu chí phía dưới để hoàn tất đặt combo này!
                                    </p>
                                </div>
                                <div

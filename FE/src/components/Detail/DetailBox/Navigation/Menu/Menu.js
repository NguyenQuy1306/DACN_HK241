...
const Menu = ({ selectedPlace }) => {
    ...
    const handleAddItem = (item, quantity) => {
        if (quantity > 0) {
            const existingItem = newCombo.find((comboItem) => comboItem.item.ten === item.ten);
            if (existingItem) {
                setNewCombo((prev) =>
                    prev.map((comboItem) =>
                        comboItem.item.ten === item.ten ? { ...comboItem, quantity: quantity } : comboItem,
                    ),
                );
            } else {
    ...
    const currentFoodItems = foodDatafromAPI.slice(indexOfFirstItem, indexOfLastItem);
                ...
                <div className="MenuNavBar_menu">
                    <div className="MenuNavBar_menu_H1">
                        ...
                        <p className="MenuNavBar_menu_H1_p">
                            <span>
                                Khám phá thực đơn lựa chọn của đầu bếp với nhiều món ăn đa dạng với mức giá cố định. Một
                                số có thể đặt qua TheFork, trong khi những món khác có thể tạo combo hoặc sẵn tại chỗ.
                            </span>
                        </p>
                    ...
                    <div className="MenuNavBar_createMenu">
                        ...
                        <div className="MenuNavBar_createMenu_div">
                            {currentFoodItems.map((category, index) => (
                                <div
                                    key={index}
                                    className="MenuNavBar_createMenu_div_index"
                                >
